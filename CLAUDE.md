...
- Lancer via Docker : `docker compose run --rm xoa-cli`
- Tests : `uv run pytest`
- Console de debug Textual : `uv run textual console` dans un terminal, puis `uv run textual run --dev src/xoa_cli/app.py` dans un autre
...
## Architecture
- `src/xoa_cli/app.py` — `XoaApp`, racine Textual ; au montage, charge la config et route vers l'écran de connexion ou le dashboard.
- `src/xoa_cli/config.py` — `Config` (host, ssh_user, ssh_port) persistée dans `~/.config/xoa-cli/config.toml` en `0600`. Le serveur et le login sont demandés une seule fois puis mémorisés.
- `src/xoa_cli/ssh.py` — `run_remote_command()` : exécution asynchrone d'une commande sur l'hôte distant, renvoie `CommandResult` (returncode, stdout, stderr).
- `src/xoa_cli/screens/connect.py` — saisie serveur / login SSH, sauvegarde la config et rend la main à l'app.
- `src/xoa_cli/screens/dashboard.py` — écran principal : boutons d'actions (`COMMANDS`) et sortie des commandes dans un `RichLog`.
- `src/xoa_cli/app.tcss` — feuille de style Textual.
